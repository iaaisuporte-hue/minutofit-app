...
export function MetabolicCheckinModal({ open, onClose, onSave }: Props) {
      ...
      <form className="metabolic-modal-panel" onSubmit={submit}>
        ...
        <section style={{ display: 'grid', gap: 'var(--space-3)' }}>
          ...
          <div className="metabolic-form-grid">
            <label className="metabolic-field"><span>Peso (kg)</span><input type="number" step="0.1" inputMode="decimal" value={form.weightKg} onChange={(e) => setField('weightKg', e.target.value)} /></label>
            <label className="metabolic-field"><span>Cintura (cm)</span><input type="number" step="0.1" inputMode="decimal" value={form.waistCm} onChange={(e) => setField('waistCm', e.target.value)} /></label>
        ...
        <section style={{ display: 'grid', gap: 'var(--space-3)' }}>
          ...
          <div className="metabolic-form-grid">
            <label className="metabolic-field"><span>Sistólica</span><input type="number" inputMode="numeric" value={form.systolicMmhg} onChange={(e) => setField('systolicMmhg', e.target.value)} /></label>
        ...
        <section style={{ display: 'grid', gap: 'var(--space-3)' }}>
          ...
          <div className="metabolic-form-grid">
            <label className="metabolic-field"><span>Jejum (mg/dL)</span><input type="number" inputMode="numeric" value={form.fastingGlucoseMgdl} onChange={(e) => setField('fastingGlucoseMgdl', e.target.value)} /></label>
          </div>
        </section>

        <label className="metabolic-field"><span>Nota opcional</span><textarea value={form.notes} onChange={(e) => setField('notes', e.target.value)} placeholder="Ex.: semana com sono melhor, rotina mais leve..." /></label>
